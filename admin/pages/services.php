...
define('SVC_DEFAULT', '/hms/admin/images/default.jpg');
?>
...
<!-- Form -->
<h4 id="formTitle" style="margin-bottom:16px;font-size:1rem;color:#1a3c5e;">➕ Add New Service</h4>
...
<div id="serviceForm">
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
        ...
        <div>
            <label style="font-size:0.78rem;font-weight:600;color:#555;display:block;margin-bottom:5px;">Service Name *</label>
            <input type="text" id="name" placeholder="e.g. Intensive Care Unit"
...
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;">
    <h4 style="font-size:1rem;color:#1a3c5e;">All Services</h4>
    <span id="serviceCount" style="background:#e8f0f8;color:#1a3c5e;font-size:0.78rem;font-weight:700;padding:3px 12px;border-radius:20px;">
            ...
            <tr id="row-<?= $s['id'] ?>" style="border-bottom:1px solid #eef1f5;">
                ...
                <td style="padding:11px 14px;">
                    <div style="display:flex;gap:8px;">
                        ...
                        <button onclick="deleteService(<?= $s['id'] ?>, '<?= htmlspecialchars(addslashes($s['name'])) ?>')"
...
function reloadTable() {
    ...
    .then(res => {
        ...
        const count = res.services.length;
        document.getElementById('serviceCount').textContent = count + ' service' + (count !== 1 ? 's' : '');

        if (!count) {
            tbody.innerHTML = '<tr><td colspan="6" style="padding:40px;text-align:center;color:#aaa;">No services yet.</td></tr>';
            return;
        }
            ...
            <tr id="row-${s.id}" style="border-bottom:1px solid #eef1f5;">
                ...
                <td style="padding:11px 14px;">
                    <div style="display:flex;gap:8px;">
                        <button onclick="editService(${s.id})"
                            style="padding:6px 14px;background:#e8f4ff;color:#1a6fcc;border:1px solid #b8d8f8;border-radius:5px;font-size:0.8rem;font-weight:600;cursor:pointer;">Edit</button>
                        <button onclick="deleteService(${s.id}, '${esc(s.name)}')"
                            style="padding:6px 14px;background:#fff0f0;color:#cc2233;border:1px solid #f8c8cc;border-radius:5px;font-size:0.8rem;font-weight:600;cursor:pointer;">Delete</button>
                    </div>
                </td>
            </tr>
